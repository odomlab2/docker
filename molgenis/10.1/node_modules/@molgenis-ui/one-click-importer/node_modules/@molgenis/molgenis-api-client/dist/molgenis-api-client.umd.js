(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('babel-runtime/helpers/extends'), require('babel-runtime/core-js/promise'), require('isomorphic-fetch'), require('lodash'), require('form-data')) :
  typeof define === 'function' && define.amd ? define(['babel-runtime/helpers/extends', 'babel-runtime/core-js/promise', 'isomorphic-fetch', 'lodash', 'form-data'], factory) :
  (global.@molgenis/molgenis-api-client = factory(global._extends,global._Promise,global.isomorphicFetch,global.lodash,global.FormData));
}(this, (function (_extends,_Promise,isomorphicFetch,lodash,FormData) { 'use strict';

_extends = 'default' in _extends ? _extends['default'] : _extends;
_Promise = 'default' in _Promise ? _Promise['default'] : _Promise;
FormData = 'default' in FormData ? FormData['default'] : FormData;
 ...
 */
var mergeOptions = function mergeOptions(method, options, force) {
  return force ? _extends({}, options, { method: method }) : lodash.merge({ method: method }, defaultOptions, options);
};
 ...
 * defaults to false.
 */
var get = function get(url, options_, forceOptions) {
  return fetch(url, mergeOptions('GET', options_, forceOptions)).then(handleResponse).then(function (response) {
    return response;
  });
};
 ...
 * defaults to false.
 */
var post = function post(url, options_, forceOptions) {
  return fetch(url, mergeOptions('POST', options_, forceOptions)).then(handleResponse).then(function (response) {
    return response;
  });
};
 ...
 */
var put = function put(url, options_, forceOptions) {
  return fetch(url, mergeOptions('PUT', options_, forceOptions)).then(handleResponse).then(function (response) {
    return response;
  });
};
 ...
 * @param forceOptions optional boolean if set to true options are not merged by used as passed, defaults to false
 */
var delete_ = function delete_(url, options_, forceOptions) {
  return fetch(url, mergeOptions('DELETE', options_, forceOptions)).then(handleResponse).then(function (response) {
    return response;
  });
};
...
return molgenisApiClient;

})));
//# sourceMappingURL=molgenis-api-client.umd.js.map
